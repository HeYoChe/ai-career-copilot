...
**🔗 在线体验** → https://ai-career-copilot-yfgzfnjjh56ujpkdexgvpj.streamlit.app/

> 一个基于 **NLP 语义匹配 + 技能缺口分析** 的求职助手：把你的简历和目标岗位 JD 丢进来，
> 它会给出匹配度评分、缺失技能清单、ATS 体检结果、个性化面试题和简历优化建议。

**首次打开提示**：免费实例长时间无人访问会自动休眠，若看到唤醒按钮点一下即可。第一次点「开始分析」需要下载约 90MB 的语义模型（约 1~2 分钟），之后会明显变快。不配置 API Key 也能使用全部离线功能，详见下方[双模式设计](#双模式设计)。
...
| 🔑 **技能缺口分析** | 内置 9 大类、84 项中英文技能词典，输出「岗位要求 / 已具备 / 缺失」三组技能 + 覆盖率 |
